...
def export_metadata_v7():
    vec_path = base_dir / "models" / "tfidf_vectorizer_v7.pkl"
    ...
    # Binary/Multiclass check
    if len(class_order) == 2:
        weights = model.coef_[0].tolist()
    else:
        # Multiclass: find index of 'DANGEROUS'
        try:
    ...
    metadata = {
        ...
        "training_schema": "semantic_risk_v7",
    ...
    with open(models_dir / "tfidf_vocabulary_v7.json", "w", encoding="utf-8") as f:
        json.dump(vocabulary, f, ensure_ascii=False)
        
    with open(models_dir / "tfidf_idf_v7.json", "w", encoding="utf-8") as f:
        json.dump(idf_weights, f)
        
    with open(models_dir / "class_map_v7.json", "w", encoding="utf-8") as f:
        json.dump(class_map, f)
        
    with open(models_dir / "xai_weights_v7.json", "w", encoding="utf-8") as f:
        json.dump(weights, f)
        
    with open(models_dir / "model_metadata_v7.json", "w", encoding="utf-8") as f:
        json.dump(metadata, f)
        
    print("V7 Metadata export complete (including XAI weights).")
